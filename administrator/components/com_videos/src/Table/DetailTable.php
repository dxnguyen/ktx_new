...
namespace Videos\Component\Videos\Administrator\Table;
// No direct access
...
use \Videos\Component\Videos\Administrator\Helper\VideosHelper;
...
class DetailTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_videos.detail';
	...
	public function bind($array, $ignore = '')
	{
		...
		// Support for checkbox field: is_hot
		if (!isset($array['is_hot']))
		{
			$array['is_hot'] = 0;
		}
		...
		if (!$user->authorise('core.admin', 'com_videos.detail.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_videos/access.xml',
				"/access/section[@name='detail']/"
			);
			$default_actions = Access::getAssetRules('com_videos.detail.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['image'][0]['size'] > 0)
		{
			...
			$oldFiles = VideosHelper::getFiles($this->id, $this->_tbl, 'image');

			foreach ($oldFiles as $f)
			{
				$oldFile = JPATH_ROOT . '/uploads/videos/' . $f;
			...
			foreach ($files['image'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '/uploads/videos/' . $filename;
					$fileTemp = $singleFile['tmp_name'];
                    $uploadPathToThumbs = JPATH_ROOT.'/uploads/videos/thumb/' . $filename;
	...
	protected function _getAssetParentId($table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_videos');
    ...
    public function delete($pk = null)
    {
		...
		if ($result)
		{
			...
			{
			case 'string':
				File::delete(JPATH_ROOT . '/uploads/videos/' . $this->image);
			break;
			default:
			foreach ($this->image as $imageFile)
			{
				File::delete(JPATH_ROOT . '/uploads/videos/' . $imageFile);
			}
			}
		}
