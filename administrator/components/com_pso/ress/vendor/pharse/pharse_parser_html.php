...
class HTML_Parser_Base extends Tokenizer_Base
{
    ...
    /**
     * Parse an HTML string (attributes)
    ...
    /**
     * Parse content up to closing tag
     * @param string $closing_tag
     * @return bool
     */
    protected function parse_up_to($closing_tag)
    {
        if ($this->parse_attributes() && ($this->token === self::TOK_TAG_CLOSE) && ($start = $this->pos) && ($this->next_pos($closing_tag, false) === self::TOK_UNKNOWN)) {
            $len = $this->pos - 1 - $start;
            $this->status['text'] = (($len > 0) ? substr($this->doc, $start + 1, $len) : '');
    ...
    /**
     * Parse math tags (MathML)
     * @return bool
     */
    protected function parse_math()
    {
        if ($this->parse_up_to('</math>')) {
            return true;
        }
        $this->addError('No end for math tag found');
        return false;
    }
...
//                $val = $attr;
                $val = false;
                $this->pos = ($this->token_start ?: $this->pos) - 1;
            }

            if (!isset($attributes[$attr])) { // skip duplicates
                $attributes[$attr] = $val;
            }
        }
    ...
    protected function parse_tag()
    {
        ...
        if ($next === '!') {
            ...
            if (substr_compare($this->doc, '--', $this->pos + 2, 2) === 0) {
                $this->status['comment'] = true;

                if (($this->doc[$this->pos + 4] === '[') && substr_compare($this->doc_lc, 'if', $this->pos + 5, 2) === 0) {
            ...
            if ($this->doc[$this->pos + 2] === '[') {
                if (substr_compare($this->doc_lc, 'if', $this->pos + 3, 2) === 0) {
                    return $this->parse_conditional();
                }
                if (substr_compare($this->doc_lc, 'endif', $this->pos + 3, 5) === 0) {
                    $this->status['closing_tag'] = true;
                    return $this->parse_conditional();
                }
                if (substr_compare($this->doc_lc, 'cdata', $this->pos + 3, 5) === 0) {
...
class HTML_Parser extends HTML_Parser_Base
{
     ...
     * @internal This is where most of the nodes get added
     * @access private
     */
