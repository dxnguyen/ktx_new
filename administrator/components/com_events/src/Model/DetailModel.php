...
namespace Events\Component\Events\Administrator\Model;
// No direct access.
...
class DetailModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_events.detail';
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_events.edit.detail.data', array());
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_events'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__events');
				$max             = $db->loadResult();
