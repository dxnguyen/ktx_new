...
namespace Slideshows\Component\Slideshows\Administrator\Table;
// No direct access
 ...
 * @since 1.0.0
 */
class BannerTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_slideshows.banner';
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!$user->authorise('core.admin', 'com_slideshows.banner.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_slideshows/access.xml',
				"/access/section[@name='banner']/"
			);
			$default_actions = Access::getAssetRules('com_slideshows.banner.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['image'][0]['size'] > 0)
		{
			...
			$oldFiles = SlideshowsHelper::getFiles($this->id, $this->_tbl, 'image');

			foreach ($oldFiles as $f)
			{
				$oldFile = JPATH_ROOT . '/uploads/banners/' . $f;
			...
			foreach ($files['image'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '/uploads/banners/' . $filename;
	...
	protected function _getAssetParentId($table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_slideshows');
    ...
    public function delete($pk = null)
    {
		...
		if ($result)
		{
			...
			{
			case 'string':
				File::delete(JPATH_ROOT . '/uploads/banners/' . $this->image);
			break;
			default:
			foreach ($this->image as $imageFile)
			{
				File::delete(JPATH_ROOT . '/uploads/banners/' . $imageFile);
			}
			}
		}
