...
namespace Partners\Component\Partners\Administrator\Table;
// No direct access
...
use \Partners\Component\Partners\Administrator\Helper\PartnersHelper;
...
class DetailTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_partners.detail';
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!$user->authorise('core.admin', 'com_partners.detail.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_partners/access.xml',
				"/access/section[@name='detail']/"
			);
			$default_actions = Access::getAssetRules('com_partners.detail.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['image'][0]['size'] > 0)
		{
			...
			$oldFiles = PartnersHelper::getFiles($this->id, $this->_tbl, 'image');

			foreach ($oldFiles as $f)
			{
				$oldFile = JPATH_ROOT . '/uploads/partners/' . $f;
			...
			foreach ($files['image'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '/uploads/partners/' . $filename;
	...
	protected function _getAssetParentId($table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_partners');
    ...
    public function delete($pk = null)
    {
		...
		if ($result)
		{
			...
			{
			case 'string':
				File::delete(JPATH_ROOT . '/uploads/partners/' . $this->image);
			break;
			default:
			foreach ($this->image as $imageFile)
			{
				File::delete(JPATH_ROOT . '/uploads/partners/' . $imageFile);
			}
			}
		}
