...
/**
 * Methods supporting a list of Homepage records.
 *
 * @since  1.0.0
 */
class SearchModel extends ListModel
{
    ...
    public function getSearch($keyword)
    {
        ...
        $query->select('*')
            ...
            ->where($db->qn('c.title'). ' LIKE ' . $db->q('%' .trim($keyword). '%'))
            ->orWhere($db->qn('c.introtext'). ' LIKE ' . $db->q('%' .trim($keyword). '%'))
            ->orWhere($db->qn('c.fulltext'). ' LIKE ' . $db->q('%' .trim($keyword). '%'))
