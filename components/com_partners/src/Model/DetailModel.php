...
namespace Partners\Component\Partners\Site\Model;
// No direct access.
...
use \Partners\Component\Partners\Site\Helper\PartnersHelper;
...
class DetailModel extends ItemModel
{
	...
	{
		$app  = Factory::getApplication('com_partners');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_partners')) && (!$user->authorise('core.edit', 'com_partners')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_partners.edit.detail.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_partners.edit.detail.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_PARTNERS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_PARTNERS_ITEM_NOT_LOADED'), 404);
			}
		}
