...
namespace Videos\Component\Videos\Site\Model;
// No direct access.
...
use \Videos\Component\Videos\Site\Helper\VideosHelper;
...
class DetailModel extends ItemModel
{
	...
	{
		$app  = Factory::getApplication('com_videos');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_videos')) && (!$user->authorise('core.edit', 'com_videos')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_videos.edit.detail.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_videos.edit.detail.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_VIDEOS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_VIDEOS_ITEM_NOT_LOADED'), 404);
			}
		}
