...
namespace Events\Component\Events\Site\Model;
// No direct access.
...
use \Events\Component\Events\Site\Helper\EventsHelper;
...
class DetailModel extends ItemModel
{
	...
	{
		$app  = Factory::getApplication('com_events');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_events')) && (!$user->authorise('core.edit', 'com_events')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_events.edit.detail.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_events.edit.detail.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_EVENTS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_EVENTS_ITEM_NOT_LOADED'), 404);
			}
		}
