...
namespace Comments\Component\Comments\Site\Model;
// No direct access.
...
use \Comments\Component\Comments\Site\Helper\CommentsHelper;
...
class DetailModel extends ItemModel
{
	...
	{
		$app  = Factory::getApplication('com_comments');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_comments')) && (!$user->authorise('core.edit', 'com_comments')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_comments.edit.detail.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_comments.edit.detail.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_COMMENTS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_COMMENTS_ITEM_NOT_LOADED'), 404);
			}
		}
